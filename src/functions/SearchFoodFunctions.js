...
const getArrBreakfast = (columns, arrFoods ) => {
  ...
  const getArrInformation = (percenProt, addProteins,percenLip, addLipids, percenCarb, addHc, addKcal, date, nintCards,arrFoods,addOuputsFoods ) => {
    let arrInformation = [
      {
        proteins:
          ...
          "g",
        fats:
          Math.round(percenLip(addLipids, addKcal)) + "%" + " / " + Math.round(addLipids()) + "g",
        carbohydrates:
          Math.round(percenCarb(addHc, addKcal)) + "%" + " / " + Math.round(addHc(nintCards,arrFoods,addOuputsFoods)) + "g",
...
export {addFood, allDelete, getArrBreakfast, getArrLunch, getArrDinner, getArrSnack, getArrInformation, exportPDF  };
